    ...
    <div className="h-screen w-full bg-gradient-to-b from-black via-black to-gray-800">
      <div className="mx-auto flex flex-col items-center justify-between h-full px-10 md:flex-row">
        ...
        <div className="flex flex-col justify-center h-full pr-20 md:w-2/4 ml-10">
          <h2 className="text-7xl font-semibold text-white mb-4">
            Upcoming Events
          </h2>
          ...
          <div className="flex w-full flex-wrap h-full md:h-1/4">
            {events.map(({ id, title, link }) => {
              return (
                <div
                  key={id}
                  className="py-4 w-1/4 flex justify-start items-center p-2 rounded-2xl"
                >
                  <Link to={`/${link}`}>
                    <p className="cursor-pointer text-3xl text-gray-400 font-medium hover:scale-105 duration-200 hover:text-white">
                      {title}
                    </p>
                  </Link>
                </div>
